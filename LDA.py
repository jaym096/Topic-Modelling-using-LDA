...
def return_words_in_doc(filename):
...
#    print("docs",num_of_doc1s)
    
    num_of_word = len(all_duplicate_words)
#    print("word.",num_of_word)
...
#    wn=list(range(0,len(all_unique_words)))
    pi_n=list(range(0,len(all_duplicate_words)))
    random.shuffle(pi_n)
#    print(pi_n)
#    print(len(pi_n), min(pi_n), max(pi_n))
#    sys.exit()
...
#    sys.exit()
    ...
    for i in range(len(all_duplicate_words)):
        ...
        ct[z_n[i]][w_n[i]]+=1
#    for i in range(len(all_duplicate_words)):
#        ct[z_n[i]][all_unique_words.index(all_duplicate_words[i])]+=1
#    print(cd)
#    print(ct)
#    sys.exit()
    ...
    for i in range(0,n_iters):
        for n in range(0,num_of_word):
            ...
            for k in range(0,K): 
#                print(ct[k][word], cd[doc][k])
#                print(doc,k)
#                a=(ct[k][word] + beta) * (cd[doc][k] + alpha)
#                b=(num_of_word * beta + np.sum(ct[k,:])) * (K * alpha + np.sum(cd[doc,:]))
                p[k] =  (ct[k][word] + beta) * (cd[doc][k] + alpha) / ((len(all_unique_words) * beta + np.sum(ct[k,:])) * (K * alpha + np.sum(cd[doc,:]))) 
#              p[k]= (ct[k][word] if ct[k][word]!=0 else 0 + beta)*(cd[doc][k] if cd[doc][k]!=0 else 0 + alpha) / ( (num_of_word * beta + np.sum(ct[k])) * (K * alpha + np.sum(cd[doc])) )
#            if(np.sum(p)!=0):
#                p=p/np.sum(p)
...
if __name__ == "__main__":
    start=time.time()
    doc_word_dict,label_dict = parse_data(data_folder+sys.argv[1])
...
#    for i in range(len(all_duplicate_words)):
#        if(all_duplicate_words[i] not in topic_word_dict[z_n[i]]):
#            topic_word_dict[z_n[i]][all_duplicate_words[i]] = 0
#        else:    
#            topic_word_dict[z_n[i]][all_duplicate_words[i]] += 1
#            
#    output={topic:[] for topic in range(ct.shape[0])}
#    for topic in topic_word_dict:
#        sorted_x = sorted(topic_word_dict[topic].items(), key=lambda x: x[1], reverse=True)
#        for a in sorted_x[0:3]:
#            output[topic].append(a[0])
#            
#        print(sorted_x[0:3])
#    
#    print(output)
#    print(cd)
#    topic_word_dict={topic:[] for topic in range(ct.shape[0])}
#    for i in range(len(ct)):
#        c=list(ct[i])
#        temp=sorted(ct[i],reverse=True)
#        temp=temp[0:3]
#        for j in range(len(temp)):
#            topic_word_dict[i].append(all_unique_words[c.index(temp[j])])
...
#    print(topic_word_dict)
    ...
    for i in range(cd.shape[0]):
        for j in range(0,K):
            first_representation[i][j] = first_representation[i][j] + alpha / (K * alpha + np.sum(first_representation[i,:])  ) 
...
#    print(first_representation)
#    print(second_representation)
